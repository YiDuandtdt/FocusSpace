...
> 第五阶段增量（2026-09-08）：个人历史、任务标题公开级别、房间进度、共同专注、聊天恢复、轻互动及偏好已实现。与下文 MVP 基线的“后续/暂不支持”表述冲突时，以 [第五阶段交付与规则](阶段交付_共学体验与学习反馈.md) 为准；原恢复与结算架构不变。
...
主动离开立即关闭个人计时区间。房主主动离开先选择在线且非 AFK 的有效成员接任，无合适成员才结束全房；房主最后一个连接断开后允许 60 秒恢复，超时同样先接任。主动点击“结束共学”仍结束全房。其他成员短时断线不改变房间节奏。每个连接发送轻量心跳，服务端只在整个用户的连接集合失活时改变成员在线状态。
...
进程重启后，从数据库恢复未结束 Session，按已保存的时间轴计算当前阶段。在线资格全部重新确认，成员离线期间不补记个人专注时长；房主在恢复宽限期内未回来时，优先由已恢复在线且非 AFK 的成员接任，无人可接任才结束。阶段恢复和成员失效处理先完成，再对外提供可操作快照。
...
鉴权、房主权限、任务归属、阶段限制均由服务端执行。房间码只是加入入口，不能替代登录或成员校验；加入尝试与登录做基本限流。日志记录 requestId、roomId、操作和错误原因，不打印密码、令牌及私有任务正文。管理员封禁或强制结束必须写 AuditLog，第六阶段管理员面板已交付，成功变更和审计同事务提交。
